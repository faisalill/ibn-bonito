...
#     viterbi_alignments,
#     logZ_cu_sparse,
#     bwd_scores_cu_sparse,
#     fwd_scores_cu_sparse,
# )
...
class CTC_CRF(SequenceDist):
    ...
    def viterbi(self, scores):
        ...
        paths = 1 + (
            torch.div(a_traceback, len(self.alphabet), rounding_mode="floor")
            % self.n_base
        )
...
def rnn_encoder(
    ...
    return Serial(
        [
            ...
            conv(
                first_conv_size, 16, ks=5, bias=True, activation=activation, norm=norm
            ),
            conv(
                16,
                features,
            ...
            Permute([2, 0, 1]),
            *(
                rnn(features, features, reverse=(num_layers - i) % 2)
...
class SeqdistModel(Module):
    def __init__(
        self, encoder, seqdist, n_pre_post_context_bases=None, target_projection=None
    ):
        super().__init__()
        ...
        if n_pre_post_context_bases is None:
            ...
            self.n_pre_context_bases, self.n_post_context_bases = (
                n_pre_post_context_bases
            )
        ...
        if target_projection is None:
            ...
            self.register_buffer(
                "target_projection",
                torch.tensor([0] + target_projection),
                persistent=False,
            )
    ...
    def to_dict(self, include_weights=False):
        ...
        res = {
            ...
            "seqdist": {
                "state_len": self.seqdist.state_len,
                "alphabet": self.seqdist.alphabet,
            },
            "n_pre_post_context_bases": (
                self.n_pre_context_bases,
                self.n_post_context_bases,
            ),
        }
        if self.target_projection is not None:
